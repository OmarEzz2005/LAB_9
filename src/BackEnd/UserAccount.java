...
public class UserAccount {
    ...
    public ArrayList<UserAccount> ConvertToObject(ArrayList<String> before)
    {
        ArrayList<UserAccount> After=null;
        for (UserAccount user : LOGIN.database.getUsers())
        {
            if(before.contains(user.getUsername()))
            {
                After.add(user);
            }
        }
   ...
   /* public ArrayList<UserAccount> getFriendSuggestions() {
    ...
    ArrayList<UserAccount> UserFriendList = new ArrayList<>();
    for (String friendUsername : this.getFriends()) {
        UserAccount friend = LOGIN.database.getRecord(friendUsername);
        if (friend != null) {
    ...
    for (UserAccount friend : UserFriendList) {
        for (String fof : friend.getFriends()) {
            if (!this.getFriends().contains(fof) && !fof.equals(this.getUsername())) {
                UserAccount mutualFriend = LOGIN.database.getRecord(fof);
                if (mutualFriend != null && !mutualFriends.contains(mutualFriend)) {
...
public ArrayList<UserAccount> getFriendSuggestions() {
    ...
    if (this.getFriends() == null || this.getFriends().isEmpty()) {
        for (UserAccount user : LOGIN.database.getUsers()) {
            ...
            if (suggestedFriends.size() == 5) break; 
        }
    } else {
